...
-(void)appendText:(NSString*)s {
	NSAttributedString* as = [[NSAttributedString alloc]
		initWithString:s attributes:m_attr1];
	[m_result appendAttributedString:as];
}

-(void)appendValue:(NSString*)s {
	NSAttributedString* as = [[NSAttributedString alloc]
		initWithString:s attributes:m_attr2];
	[m_result appendAttributedString:as];
}

-(void)appendImportantValue:(NSString*)s {
	NSAttributedString* as = [[NSAttributedString alloc]
		initWithString:s attributes:m_attr3];
	[m_result appendAttributedString:as];
}

-(void)appendHeadlineText:(NSString*)s {
	NSAttributedString* as = [[NSAttributedString alloc]
		initWithString:s attributes:m_attr4];
	[m_result appendAttributedString:as];
}
...
-(void)dumpAttr {
	...
	struct AttrBuffer buf;

	int err = getattrlist(filename, &al, &buf, sizeof(struct AttrBuffer), FSOPT_NOFOLLOW);
	if(err) {
		[self appendText:@"none\n"];
		return;
	}
...
-(void)dumpSpotlightInfo {
	...
	*/
	...
	CFStringRef path = (CFStringRef)CFBridgingRetain(m_path);
	...
	*/
	...
	NSArray* array = (NSArray*)CFBridgingRelease(attributeNames);
	// NSLog(@"%s %@", _cmd, array);
    ...
    while ((arrayObject = [e nextObject]))
    {
        ...
        NSObject* tempObject = (NSObject*)CFBridgingRelease(ref);
